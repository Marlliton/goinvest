...
É uma ferramenta pessoal, de uso individual, feita para um investidor iniciante que quer decidir melhor _e_ aprender no processo — não um produto multiusuário nem um robô de recomendação.
...
- **O caso de uso é minúsculo:** ~8 URLs conhecidas por execução, sem descoberta de links, sem crawl recursivo. Colly resolve _crawling_; você faz _fetching_.
...
A pergunta que pega o erro: _o número é calculável, mas a pergunta que ele responde faz sentido
para este ativo?_ Quando não faz, mostrar ausência com motivo, nunca o número. Cuidado com o
...
- **`source/` esconde HTML _e_ JSON atrás da mesma interface** — o requisito de fallback do PROJECT.md depende disso.
...
- O schema append-only já constrói série histórica _incrementalmente_ a partir de hoje
...
- **Comentar é exceção, não hábito.** O padrão é código sem comentário; o comentário precisa justificar a própria existência.
- Comentar só quando o _porquê_ não é dedutível do código: comportamento contraintuitivo de biblioteca ou ferramenta, decisão de tipo que parece firula sem contexto, caractere invisível, linha que um leitor futuro apagaria por parecer inútil.
- **Nunca** comentar o que o código já diz (`// separador de milhar` em cima de um `ReplaceAll(".", "")`), nem escrever doc de pacote em forma de ensaio.
- **Nunca** citar artefato de planejamento em comentário: nada de `Armadilha 9`, `D-03`, `Critério 5 do ROADMAP`, `ver RESEARCH.md`, número de plano ou de fase. O código sobrevive ao planejamento.
- **Não usar travessão (`—`) em comentários.** Usar dois-pontos, ponto ou parênteses.
